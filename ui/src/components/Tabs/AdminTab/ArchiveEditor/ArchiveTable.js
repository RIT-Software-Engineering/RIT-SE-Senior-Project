...
export default function ArchiveTable(props) {
  ...
  let handleSearchChange = (e, { value }) => {
    ...
    SecureFetch(
      `${config.url.API_GET_SEARCH_FOR_ARCHIVES}/?resultLimit=${PROJECTS_PER_PAGE}&offset=${0}&searchQuery=${input}&inactive=true`,
    )
          ...
          <TableBody key={projects}>
            {projects?.map((project, idx) => {
              ...
              let tags = [
                ...
                project.creative ? "creative" : null,
                project.outstanding ? "outstanding" : null,
              ];
              return (
                <TableRow
                  className={
                    project.inactive === null || project.inactive === ""
                      ? ""
                      : "proposal-row-gray"
                  }
                  key={idx}
