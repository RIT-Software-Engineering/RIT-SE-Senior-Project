...
export default forwardRef(function GanttChartBackdrop(props, todayRef) {
  ...
  let monthLength = daysInMonth(
    startCol.getUTCMonth(),
  ...
  for (let i = 1; i < cols; i++) {
    ...
    if (props.timeSpan === "week") {
      ...
      if (currDate === monthLength + 1) {
        ...
        monthLength = daysInMonth(currMonth, currYear);
        if (i + monthLength > cols) {
          // to cut off the month at the end of the calendar (not really necessary)
      ...
      if (currDate === monthLength + 1) {
        ...
        monthLength = daysInMonth(currMonth, currYear);
        if (i + monthLength > cols) {
          // to cut off the month at the end of the calendar
          monthLength = monthLength - (i + monthLength - cols);
        }
  ...
  return (
    <>
      <div
        ...
        style={{ gridColumn: "2/" + (props.ganttLength + 1) }}
      >
        {ganttHeader}
      </div>
      <div
