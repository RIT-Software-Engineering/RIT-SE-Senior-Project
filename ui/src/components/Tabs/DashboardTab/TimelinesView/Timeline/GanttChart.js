...
export default function GanttChart(props) {
  ...
  useEffect(() => {
    if (semesterActive && firstAction) {
      try {
        let viewTop =
          firstActionRef.current.offsetTop - todayRef.current.offsetTop;
        let viewLeft = todayRef.current.offsetLeft - sidebarWidth;
        containerRef.current.scrollTo(viewLeft, viewTop);
      } catch (e) {
        console.log(
          "issue with snapping to current day (x), first action (y)",
          e,
        );
      }
    }
  ...
  leftSideRows.push(
    <div key={"sidebar header"} className="sidebar header">
      Name
    </div>,
  );
  ...
  sortedActions.forEach((action, idx) => {
    ...
    const leftRowButton = (
      <button className="sidebar" style={{ gridRow: gridrow }}>
        {action.action_title}
      </button>
    );
    let leftRow;
  ...
  if (isMobile()) {
    ...
    ganttSideContainer = (
      <div className="sidebar-container">{leftSideRows}</div>
    );
  }
