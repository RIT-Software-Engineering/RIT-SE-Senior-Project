...
export default function SponsorsTab(props) {
  ...
  const getPaginationData = () => {
    SecureFetch(
      `${config.url.API_GET_ALL_SPONSORS}/?resultLimit=${LOGS_PER_PAGE}&offset=${LOGS_PER_PAGE * activePage}`,
    )
  ...
  let handleSearchChange = (e, { value }) => {
    ...
    SecureFetch(
      `${config.url.API_GET_SEARCH_FOR_SPONSOR}/?resultLimit=${LOGS_PER_PAGE}&offset=${0}&searchQuery=${value}`,
    )
      .then((response) => response.json())
      .then((results) => {
