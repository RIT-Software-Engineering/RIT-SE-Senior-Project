...
export default function EvalReview(props) {
            ...
            {Object.entries(student).map(([student_name, data]) => {
                ...
                return <Accordion fluid styled key={props.id + student_name + "Card" + index} >
                    <Accordion.Title active={studentExpanded[student_name]}
                                     index={index}
                                     onClick={() => { updateExpanded(student_name) }}>
                        <Icon name='dropdown'/>
                        ...
                        <Accordion.Content active={studentExpanded[student_name]}>
                            <Table striped>
                                <Table.Header>
                                    ...
                                    {hasSelfRating && <Table.HeaderCell>Self Rating</Table.HeaderCell>}
                                </Table.Header>
                                <Table.Body>
    ...
    return (<div>
        {props.isSub ? userFeedback.map((feedback, index) => generateFeedbackCards(feedback, index)) :
            <h5>No Submission</h5>}
    </div>)
}
