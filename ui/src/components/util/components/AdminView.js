...
export default function AdminView(props) {
  ...
  if ((user?.isMock || user?.role === "admin") && !user?.view_only) {
    return (
      <>
        <div
          className={`ui ${user?.isMock ? "warning message" : "info message"}`}
        >
          Currently signed in as: {user?.fname} {user?.lname} ({user?.user}) who
          is a "{user.role}"
        </div>
        ...
        <Dropdown
          ...
          button
          value={selectedUser}
          options={_.sortBy(Object.values(users), [
            "fname",
            "lname",
            "last_login",
          ]).map((user) => {
            return {
              text: `${user.fname} ${user.lname} (${user.system_id}) ${user.last_login}`,
