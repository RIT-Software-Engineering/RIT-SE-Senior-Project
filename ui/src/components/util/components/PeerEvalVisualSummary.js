...
const BarGraph = ({ data }) => {
  ...
  useEffect(() => {
    const sortedFeedback = Object.entries(data.Students).map(([student, feedback]) => [
      student,
      feedback.AverageRatings,
    ])
          ...
          return Object.entries(scores).map(([label, d], categoryIdx) => {
            ...
            return (
              <g key={`${personName}-${label}`}>
                ...
                />
                <text x={xPos + individualBarWidth / 2} y={yPos - 5} fontSize={12} textAnchor="middle" fill={textColor}>
                  {d}
                </text>
              </g>
            )
        ...
        {userFeedback.length > 0 &&
          Object.keys(userFeedback[0][1]).map((label, categoryIdx) => (
            <text
              ...
              x={
                padding +
                categoryIdx * ((width - 2 * padding) / Object.keys(userFeedback[0][1]).length) +
